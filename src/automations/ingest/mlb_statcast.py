"""MLB Statcast data ingestion utilities."""
...
def _fetch_game_statcast_data(game_id: int, api: MlbStatsApi) -> tuple[list[dict], list[dict]]:
    ...
    try:
        ...
        # Convert to dict rows for BigQuery
        pitch_rows = [
            {
                "play_id": p.play_id,
                "game_id": p.game_id,
        ...
        # Return empty lists on error - will be logged by caller
        return [], []
    ...
    *,
    ...
    if game_ids is None:
        ...
        games = api.list_games(
            season=season,
            game_types="R",  # Regular season only
...
def ingest_mlb_statcast_data_bigquery(
    *,
    ...
    for batch_start in range(0, total_games, batch_size):
        ...
        # Fetch data for this batch (parallelized)
