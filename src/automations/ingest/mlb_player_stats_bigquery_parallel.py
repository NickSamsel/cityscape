...
def ingest_mlb_player_game_stats_bigquery(
    *,
    ...
    # Wait for all futures to complete and resolve them
    from prefect.futures import wait
    wait(futures)
    
    # Collect all results - now the futures are resolved
    all_batting_rows = []
    all_pitching_rows = []
    
    # Results come back as a list of tuples (batting_rows, pitching_rows)
    ...
    logger.info(
        f"Fetched {len(all_batting_rows)} batting stat records and {len(all_pitching_rows)} pitching stat records"
    )
