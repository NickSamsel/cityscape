...
stats_with_metrics as (
  select
    ...
    -- Advanced
    {{ safe_divide('s.total_points', '(2 * (s.total_field_goals_attempted + 0.44 * s.total_free_throws_attempted))', none) }} as true_shooting_percentage,
    {{ safe_divide('(s.total_field_goals_made + 0.5 * s.total_three_pointers_made)', 's.total_field_goals_attempted', none) }} as effective_field_goal_percentage,
    {{ safe_divide('s.total_assists', 'nullif(s.total_turnovers, 0)', none) }} as assist_to_turnover_ratio,
    ({{ safe_divide('s.total_steals', 's.games_played', none) }} + {{ safe_divide('s.total_blocks', 's.games_played', none) }}) as defensive_plays_per_game
  from season_aggregates as s
  left join players as p
    on s.player_id = p.player_id
)

select *
from stats_with_metrics
